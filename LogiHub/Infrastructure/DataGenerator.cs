...
namespace LogiHub.Infrastructure
{
    public class DataGenerator
    {
        ...
        private static void SeedAzioni(TemplateDbContext context)
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                var tipoCasuale = tipi[Random.Next(tipi.Length)];
                var slId = slIds[Random.Next(slIds.Count)];
                var userId = userIds[Random.Next(userIds.Count)];

                context.Azioni.Add(new Azione
                {
                    Id = StableGuid($"AZIONE-{i:D4}"),
                    SemiLavoratoId = slId,
                    UserId = userId,
                    TipoOperazione = tipoCasuale,
                    Dettagli = $"Operazione automatica {i + 1}",
                    DataOperazione = BaseDate.AddDays(-Random.Next(1, 365))
                });
            }
        }
